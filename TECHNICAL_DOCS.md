...
2. "Training" title
3. Script text in scrollable teleprompter area
4. Audio level visualizer (bars)
5. Recording indicator (red dot + "Recording" / "Paused")
6. Control buttons (Pause | Record [large red] | Restart)
...
| `focusMode` | `string`| TrainingSetup         | `'accuracy'` \| `'free'`         |
...
| `handleRecordPress()` | Large red Record button    | Toggle recording state; on stop: upload audio to Supabase                 |
| `handlePausePress()`  | Pause button press         | Toggle pause state (only when recording)                                  |
| `handleRestartPress()`| Restart button press       | Reset recording state, scroll to top, clear audio level                   |
...
| ------------ | ---------- | ------- | ---------------------------------------- |
| Font Size    | 12–24      | 16      | Teleprompter text size                   |
...
| Audio Upload                | `{audioUri, scriptId, focusMode, type}`          | `{sessionId, score}`              |
...
| Audio Level Simulation    | `[isRecording, isPaused]`     | Simulate random audio level changes during recording                  |
| Auto Scroll              | `[autoScroll, isRecording, isPaused, scrollSpeed]` | Auto-scroll teleprompter if enabled |

#### Components Used

| Component            | Props                                 | Description                      |
| -------------------- | ------------------------------------- | -------------------------------- |
| `AudioLevelIndicator`| `level`, `isActive`, `barCount`       | Visual feedback for audio        |
| `PrimaryButton`      | `title`, `onPress`, `loading`         | Start/Stop recording             |
| `Typography`         | `variant`, `color`, `align`           | Headers and labels               |
| `ScrollView`         | `ref`, `showsVerticalScrollIndicator` | Teleprompter area                |
| `Modal`              | `visible`, `animationType`            | Settings panel                   |

#### Recording Flow

1. User presses large red Record button → `isRecording = true`, start audio capture (TODO Supabase)
2. Audio level animates in real-time, showing user's input
3. User can pause/resume with Pause button
4. User presses Record button again → `isRecording = false`, audio stops, upload to Supabase
5. Teleprompter text can be styled via Settings modal (font size, auto-scroll)
